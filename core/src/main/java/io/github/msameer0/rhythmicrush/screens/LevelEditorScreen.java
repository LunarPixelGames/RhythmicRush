...
public class LevelEditorScreen extends AbstractScreen {
    ...
    private void drawObjects(float canvasW, float canvasH) {
        ...
        for (LevelData.ObjectEntry e : levelData.getObjects()) {
            ...
            if (sel)
                shapes.setColor(Math.min(c.r * 1.5f, 1f), Math.min(c.g * 1.5f, 1f), Math.min(c.b * 1.5f, 1f), c.a);
    ...
    private float drawTab(Tab tab, float sideX, float curY, float canvasH, Vector2 m) {
        ...
        for (String id : tab.ids) {
            ...
            if (iy + cell > 0 && iy < canvasH) {
                ...
                tmp.setType(id);
                if (Registries.BLOCKS.has(id))
                    tmp.setBlockType(BlockType.values()[selectedBlockTypeIdx].getTextureName());
    ...
    private void handleKeys(float delta) {
        ...
        if (playtesting) {
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE) || Gdx.input.isKeyJustPressed(Input.Keys.F5))
                stopPlaytest();
            return;
        }
        ...
        if (!selection.isEmpty()) {
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.E)) for (LevelData.ObjectEntry e : selection)
                e.setRotation(((e.getRotation() - 90f) % 360f + 360f) % 360f);
        }
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.DEL) || Gdx.input.isKeyJustPressed(Input.Keys.BACKSPACE)) {
            for (LevelData.ObjectEntry e : new Array<>(selection))
                levelData.getObjects().removeValue(e, true);
            selection.clear();
        }
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.CONTROL_LEFT) && Gdx.input.isKeyJustPressed(Input.Keys.A)) {
            selection.clear();
            selection.addAll(levelData.getObjects());
        }
    ...
    private void stopPlaytest() {
        if (!playtesting) return;
        playtesting = false;
        if (ptRenderer != null) ptRenderer.dispose();
        ptRenderer = null;
        ptWorld = null;
        ptEngine = null;
        stopAndDisposeMusic();
    ...
    private void scanMusicFiles() {
        ...
        try {
            ...
            if (dir.exists()) {
                for (FileHandle f : dir.list()) {
                    String n = f.name();
                    if (n.endsWith(".mp3") || n.endsWith(".ogg") || n.endsWith(".wav"))
                        musicFiles.add(n);
                }
    ...
    private void placeObject(float wx, float wy) {
        ...
        e.setType(placementId);
        e.setX(wx);
        e.setY(wy);
        e.setSize(GRID_SIZE);
        e.setRotation(0f);
        if (Registries.BLOCKS.has(placementId))
            e.setBlockType(BlockType.values()[selectedBlockTypeIdx].getTextureName());
        levelData.getObjects().add(e);
        selection.clear();
        selection.add(e);
    }
        ...
        if (Registries.BLOCKS.has(e.getType())) {
            ...
            if (e.getBlockType() != null)
                for (BlockType t : BlockType.values())
                    if (t.getTextureName().equals(e.getBlockType())) {
                        bt = t;
                        break;
                    }
    ...
    private class EditorInput extends InputAdapter {
        ...
        public boolean keyTyped(char c) {
            ...
            if (c == '\t') {
                propField = (propField + 1) % propFieldCount;
                return true;
            }
        ...
        public boolean touchDown(int sx, int sy, int pointer, int button) {
            ...
            if (loadDialogOpen) {
                float pw = 400f, ph = 500f, px = sw / 2f - pw / 2f, py = sh / 2f - ph / 2f;
                if (ui.x < px || ui.x > px + pw || ui.y < py || ui.y > py + ph) {
            ...
            if (ui.y >= by && ui.y <= by + bh) {
                ...
                if (ui.x >= 84f && ui.x <= 84f + bw) {
                    stopAndDisposeMusic();
                    return true;
                }
                float offX = 160f;
                if (ui.x >= offX + 8f && ui.x <= offX + 8f + bw) {
                    newLevel();
                    return true;
                }
                if (ui.x >= offX + 84f && ui.x <= offX + 84f + bw) {
                    openLoadDialog();
                    return true;
                }
                ...
                float zbw = 36f;
                if (ui.x >= canvasW - zbw * 2 - 16f && ui.x <= canvasW - zbw * 2 - 16f + zbw) {
                    zoom = MathUtils.clamp(zoom * 0.8f, 0.08f, 10f);
                    return true;
                }
                if (ui.x >= canvasW - zbw - 8f && ui.x <= canvasW - zbw - 8f + zbw) {
                    zoom = MathUtils.clamp(zoom * 1.25f, 0.08f, 10f);
                    return true;
                }
            }
            ...
            if (ui.y >= TOPBAR_H && ui.y <= canvasH + TOPBAR_H && ui.x < canvasW) {
                ...
                if (button == Input.Buttons.MIDDLE || Gdx.input.isKeyPressed(Input.Keys.ALT_LEFT)) {
                    panning = true;
                    panStartX = sx;
                    panStartY = sy;
                    panCamX0 = camX;
                ...
                if (button == Input.Buttons.LEFT) {
                    ...
                    if (placementId != null) placeObject(snap(wx), snap(wy));
                        ...
                        for (int i = levelData.getObjects().size - 1; i >= 0; i--) {
                            LevelData.ObjectEntry e = levelData.getObjects().get(i);
                            if (wx >= e.getX() && wx <= e.getX() + e.getSize() && wy >= e.getY() && wy <= e.getY() + e.getSize()) {
                                hit = e;
                                break;
                            }
                        }
        ...
        public boolean touchUp(int sx, int sy, int p, int b) {
            ...
            if (b == Input.Buttons.LEFT && rubberBanding) {
                ...
                if (!shift) selection.clear();
                for (LevelData.ObjectEntry e : levelData.getObjects())
                    if (e.getX() + e.getSize() > minX && e.getX() < maxX && e.getY() + e.getSize() > minY && e.getY() < maxY)
        ...
        public boolean scrolled(float ax, float ay) {
            ...
            if (ui.x >= canvasW)
                sidebarScroll = MathUtils.clamp(sidebarScroll + ay * 24f, 0f, 9999f);
            else {
                float oldZoom = zoom;
                zoom = MathUtils.clamp(zoom * (1f - ay * 0.12f), 0.08f, 10f);
                float canvasH = sh - TOPBAR_H, uiY = sh - Gdx.input.getY() - TOPBAR_H;
                float wxB = (ui.x - canvasW / 2f) / oldZoom + camX, wyB = (uiY - canvasH / 2f) / oldZoom + camY, wxA = (ui.x - canvasW / 2f) / zoom + camX, wyA = (uiY - canvasH / 2f) / zoom + camY;
                camX += wxB - wxA;
                camY += wyB - wyA;
            }
            return true;
        ...
        private void handleSidebarClick(Vector2 ui, float canvasW, float canvasH) {
            ...
            for (Tab tab : tabs) {
                ...
                for (String id : tab.ids) {
                    if (col == 0) rowY -= cell + ITEM_PAD;
                    float ix = canvasW + ITEM_PAD + col * (cell + ITEM_PAD), iy = rowY;
    ...
    private void showPropertyEditor(LevelData.ObjectEntry e) {
        ...
        if ("color_trigger".equals(e.getType())) {
            propLabels = new String[]{"BG Color (hex)", "Ground Color (hex)", "Fade Duration (s)"};
            propFieldCount = 3;
            propBuffers[0].append(e.getTriggerBgColor() != null ? e.getTriggerBgColor() : "1a1a2e");
            propBuffers[1].append(e.getTriggerGroundColor() != null ? e.getTriggerGroundColor() : "16213e");
            propBuffers[2].append(e.getFadeDuration());
        } else if ("pulse_trigger".equals(e.getType())) {
            propLabels = new String[]{"Pulse BG (hex)", "Pulse Ground (hex)", "Fade In (s)", "Hold (s)", "Fade Out (s)"};
            propFieldCount = 5;
            propBuffers[0].append(e.getPulseBgColor() != null ? e.getPulseBgColor() : "1a1a2e");
            propBuffers[1].append(e.getPulseGroundColor() != null ? e.getPulseGroundColor() : "16213e");
    ...
    private void confirmPropertyEdit() {
        ...
        if ("color_trigger".equals(propTarget.getType())) {
            propTarget.setTriggerBgColor(propBuffers[0].toString().trim().replace("#", ""));
            propTarget.setTriggerGroundColor(propBuffers[1].toString().trim().replace("#", ""));
            try {
                propTarget.setFadeDuration(Float.parseFloat(propBuffers[2].toString().trim()));
            } catch (Exception ignored) {
            }
        } else if ("pulse_trigger".equals(propTarget.getType())) {
            propTarget.setPulseBgColor(propBuffers[0].toString().trim().replace("#", ""));
            propTarget.setPulseGroundColor(propBuffers[1].toString().trim().replace("#", ""));
            try {
                propTarget.setFadeInTime(Float.parseFloat(propBuffers[2].toString().trim()));
            } catch (Exception ignored) {
            }
            try {
                propTarget.setHoldTime(Float.parseFloat(propBuffers[3].toString().trim()));
            } catch (Exception ignored) {
            }
            try {
                propTarget.setFadeOutTime(Float.parseFloat(propBuffers[4].toString().trim()));
            } catch (Exception ignored) {
            }
        } else {
            try {
                propTarget.setSize(Float.parseFloat(propBuffers[0].toString().trim()));
            } catch (Exception ignored) {
            }
            try {
        ...
        try {
            if (levelMusic == null) {
                ...
                if (fh.exists()) {
                    ...
                    levelMusic.setVolume(getGame().getSettingsManager().getMusicVolume());
